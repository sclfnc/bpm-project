...
Six nets face the same list of checks (\cref{tab:nets}): the four pools, the delivered collaboration, and one variant of it. \texttt{scripts/04\_pm4py\_report.py} runs them through \texttt{pm4py} and writes one JSON report each into \texttt{pnml/analysis/}.\footnote{Five of the results in those reports are the script's own and not Woflan's: the S-components, the Rank Theorem conditions, the free-choice verdict with its list of violations, the S-net and T-net verdicts, and the split of the not-well-handled pairs into PT and TP.}
...
\begin{table}[h]
  ...
  \begin{tabular}{@{}lcccccc@{}}
    ...
    \hspace{1em}properly initialised & \ck & \ck & \ck & \ck & \ck & \ck \\
    \addlinespace[5pt]
    \midrule[\cmidrulewidth]
    \addlinespace[2pt]
    ...
    \hspace{1em}relaxed sound & \ck & \ck & \ck & \ck & \cx & \ck \\
    \rowcolor{rowshade} \hspace{1em}\textbf{sound} & \ck & \ck & \ck & \ck & \cx & \ck \\
    \bottomrule
  \end{tabular}
  \caption{Every net the report analyses. \emph{Variant} deletes the withdrawal branch and changes nothing else; it was built to test the diagnosis, not proposed (\cref{sec:global:fix}). $N^*$ is the short-circuit of the net in its own column. The Rank Theorem row is its four non-trivial conditions, since the other two hold for any connected net with a place and a transition, and it is blank where its free-choice hypothesis fails. The pipeline reports every row but the three above \emph{sound}, which follow from the Rank Theorem for the pools and from the reachability graph for the collaborations. Only the T-invariant row carries a verdict, and in one direction: on a bounded net its absence rules soundness out, its presence proves nothing.}
  \label{tab:nets}
\end{table}
...
\textbf{Well-structuredness} fails at message passing. Every TP-handle starts at a send. Most PT-handles end on a receive, the rest at the closure and at the committee's join. One of those runs from the student's withdrawal choice to the closure, an exclusive split synchronised by an AND-join. That shape is the \textbf{deadlock}: the split delivers on one path, the join waits for both (\cref{sec:global:sound}).

\textbf{S-coverability} fails at four places of $N^*$: the counter-examiner's and the committee's sinks, and the place before each. They lie in the support of no semi-positive S-invariant, and so in no S-component. Woflan reads its S-components off an invariant basis and overcounts. It names ten places here where four lie outside every S-component, and two on the variant where none do. Those four come from putting the question place by place. The missing S-cover shows nothing on its own: both theorems that would read a verdict from it need free-choice or well-structuredness, and neither holds.

\subsubsection{Safeness and the size of the reachability graph}

\textbf{Safeness} survives, and with it boundedness. Each pool is safe alone (\cref{tab:nets}). Composition adds the 27 message places and the final place, which only the closure marks. Sixteen of the message places lie on no circuit, and neither do their producers, so those sends fire once per case. The other eleven are kept safe by the strict alternation of one send and one reply on the development channels and by the propose-and-wait shape of the date negotiation itself (\cref{sec:models}). That exhausts what composition adds, and none of those places reaches two tokens.

A marking of the composition is one marking per pool plus the messages in flight. Each pool's share of it, when not empty, is a marking that pool reaches alone. Three pools have no source place inside the collaboration (\cref{tab:consistency}), so their empty state takes the slot their standalone initial marking held and their counts do not move. The student keeps its source and gains the empty marking the closure leaves behind. The four graph sizes of \cref{tab:nets} then bound the shares: at most $26 \cdot 16 \cdot 10 \cdot 12 = 5.0 \cdot 10^4$ combinations, a bound on those and not on $|V|$. On $N^*$ the graph has $|V| = 712$, so at most one combination in seventy is ever reached, and $|E| = 1983$ gives 2.8 arcs per marking. Synchronisation is what collapses the count: a pool advances only when its partner has moved. The pools do not take turns, since in most markings all four hold a token and still cannot move independently.
...
\begin{itemize}
  ...
  \item \textbf{Delete the withdrawal branch.} The net is then sound (\cref{tab:nets}): the S-cover and both invariants return, while free-choice and well-structuredness stay broken. Message passing is what breaks them, and the withdrawal accounts for one of the ten pairs. It invents no round and no channel, and the case it removes sits ahead of the core, before a topic is even picked. But the assignment states that case in the same sentence as the one it keeps, so the net is sound but not faithful to it.
\end{itemize}
